...
兼容性（**本段声明已被第二轮 review 两度修正**，实际行为以
`docs/REVIEW_ROUND2_2026-09-05.md` 为准）：旧 checkpoint 不再"自动重跑分
支"——恢复前 `prepare_graph_run` 做安全检查：旧版执行状态（共享 messages
通道有 AI/Tool 流量且分支通道全空）**无论停在哪个阶段**——待执行分支节
点、汇合屏障、已越过汇合点的辩论/交易/风控、乃至执行完毕的终态——一律
明确拒绝并保留断点（旧 OR 拓扑无法证明报告完整且屏障状态兼容）；新图首
次调用在产出消息前失败（两侧通道均无流量）属合法重试，正常恢复；新图的
下游断点（辩论/交易/风控阶段，共享通道含其 AIMessage 但分支通道有内容）
正常恢复。拓扑断言测试无需修改（join 屏障的可视图与逐条边相同）。
...
藏；收益窗口实际结束日（`end=`）≤ as_of 时收益数字可见；窗口跨 as_of 或旧格
式缺时间元数据时仅注入决策文本并注明（保守规则：无法证明可用即视为未来）。
时间语义：as_of = 该交易日收盘后（同日决策与同日收盘的收益可见）。`_fetch_
returns` 返回窗口实际结束行情日（非日历推算），回填写入 `end=`/`resolved=`
元数据。CLI/Web/个股/指数统一经 `prepare_graph_run(as_of=trade_date)`；历
史运行触发的收益回填经同一过滤不反注入。（**第二轮修正**：复盘文本的可见
性进一步以其生成日 `resolved=` ≤ as_of 为准，与收益分开判断；checkpoint
恢复时的 `past_context` 刷新在无法安全执行——并行最后写入者歧义、决策已
生成且上下文污染——时明确拒绝恢复并保留断点，不再降级为日志提示后继续。）
...
- 旧格式 resolved 条目（无 `end=`/`resolved=`）在历史模式下只注入决策文
  本（收益与复盘均剔除）：随时间推移被新回填条目自然替代，不做一次性迁
  移。
- 旧 checkpoint（第二轮起明确拒绝，不再声称自动恢复）：共享 messages 通
  道有 AI/Tool 执行流量且分支通道全空的旧断点，**无论停在哪个阶段**（分
  支节点、汇合屏障、其后的辩论/交易/风控、终态）都拒绝恢复并保留断点数
  据，用户清除该日期断点后重新开始；新图首次调用失败（无任何执行流量）
  可正常重试；新图下游阶段断点（Trader 等节点写共享 messages，但分支通
  道有内容）正常恢复。
- 断点 `past_context` 与重算不一致且最终决策已生成：拒绝恢复（决策可能
  已混入未来记忆，只擦上下文清理不掉决策文本），断点保留。
- R3 的空结果判定依赖东财 `success=true + result=null` 协议；若供应商未来变更
  无数据响应形态，`_eastmoney_datacenter` 会将其判为失败并显示数据缺失（保守
  方向失效，不会伪装成"无事件"）。
- v2 缓存命名空间下已写入的伪装空结果（"No ... data found"）无法与真实空
  结果区分，靠 v3 命名空间升级整体隔离；v1/v2 文件保留不删。
