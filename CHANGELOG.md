...
输入上证指数（`000001.SH`）/ 沪深300（`000300.SH`）/ 创业板指（`399006.SZ`）等指数，
跑与个股同构的 12 阶段多 Agent 分析（指数模式为 10 阶段）。

- **指数注册表 + 独立数据层**（`index_registry.py` / `index_data.py`，与个股 vendor 完全隔离）：
  K 线走 mootdx `index_bars`（新浪降级）、行情走腾讯、资金流走东财指数 secid（**即大盘资金流**）、
  新闻用指数中文名检索；任意指数代码（880/999/399 段）mootdx 实测探测，取不到明确报错
- **防静默出错**：000001 与平安银行同码、000688 与国城矿业同码——个股专属接口（财报/解禁/
  龙虎榜/EPS 预期/概念归属等 9 个）收到指数标识当场拒绝并说明，绝不返回同号个股数据
- **指数版 Agent**（`index_agents.py`，经 `GraphSetup(node_factories=...)` 依赖注入）：
  技术面/情绪/新闻/政策 + **大盘资金流分析师**（两市主力净流入、北向、板块轮动、赚钱效应）；
  剔除基本面/解禁；Bull/Bear/Trader/PortfolioManager 换指数框架（指数不可直接交易 →
  评级解释为经指数 ETF/期指落地的整体仓位方向，无 T+1/涨跌停/手数个股约束）
- **个股行为零改动**：裸 6 位代码语义不变（`000001` 仍是平安银行）；全部指数逻辑住新文件，
  仅 `interface.route_to_vendor`（分流）、`GraphSetup`（注入参数，默认 None=原行为）、
  `news_data_tools`（校验放宽）三处增量接缝
- **记忆与结算**：指数决策按规范标识（`000001.SH`）存储，与同号个股天然隔离；分析对象为
  沪深300 时基准自动换上证指数（避免 alpha 对自身恒 0）
- **入口**：Web 侧栏「分析类型：个股/指数/资金流向」；指数输入支持中文名（上证指数/大盘）、
  带后缀代码、指数号段裸码（探测校验）；进度面板按模式裁剪为 10 阶段

### 新增：独立资金流向报告（轻量）

`tradingagents fundflow` 子命令 / Web「资金流向」类型：拉齐指数资金流 + 北向 +
行业/概念板块主力净流入排名（东财 clist，字段 2026-08-16 实测）+ 涨停题材 + 指数新闻，
1 次模型调用整合成中文报告（成本约为完整分析的 1/20）。

### 测试

439 passed / 14 skipped / **0 failed**（新增 71 例：registry 解析、指数数据层、接缝分流、
拒绝守卫、入口解析、板块资金流、报告生成、阶段裁剪）。
