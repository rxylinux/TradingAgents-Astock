...
分析对象换成**市场指数**（上证指数/沪深300/创业板指/科创50/中证500/1000/深证成指，
或任意指数代码），走同一套多 Agent 辩论，但分析师与 prompt 全部换成指数视角
（指数技术面 / 全市场情绪 / 系统性新闻 / 政策传导 / **大盘资金流**）：

```python
from tradingagents.graph.index_graph import TradingAgentsIndexGraph

ta = TradingAgentsIndexGraph(debug=True, config=config)
final_state, decision = ta.propagate("000001.SH", "2026-08-16")   # 上证指数
# 也支持 399006.SZ（创业板指）、000300.SH（沪深300）等带交易所后缀的指数标识
```

要点：

- **指数标识必须带交易所后缀**（`000001.SH` / `399006.SZ`）——裸 `000001` 仍是个股
  （平安银行），二者全链路隔离，绝不混淆；Web/CLI 里也可以直接输中文名（「上证指数」「沪深300」）
- 任意指数代码（880/999/399 段）会经 mootdx **实测探测**校验，不是有效指数当场报错
- 个股专属数据（财报/解禁/龙虎榜等）对指数明确返回「不适用」，不会静默拿同号个股数据
- 最终评级解释为**市场整体方向性观点**（可经指数 ETF / 股指期货落地），不含个股交易约束

#### 资金流向快速报告（v0.6.0 新增）

只要一份大盘资金流诊断、不想跑完整流水线时（约 1 次模型调用，成本约为完整分析的 1/20）：

```bash
tradingagents fundflow          # CLI：向导式生成大盘资金流报告
```

或在 Web 侧栏「分析类型」选「资金流向」一键生成。报告聚合：指数主力资金流（当日分钟 +
近 20 日）、北向资金、行业/概念板块主力净流入排名、涨停题材与赚钱效应、指数新闻。
